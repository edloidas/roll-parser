...
describe( 'Map parser result to Roll:', () => {
  ...
  test( 'Should correctly map from 3 to 5 values', () => {
    ...
    expect( mapToRoll( parseSimple( '2 20 2 2 17' ))).toEqual({ dice: 20, count: 2, modifier: 2, bottom: 2, top: 17 });
    ...
    expect( mapToRoll( parseClassic( 'd10 (,7)' ))).toEqual({ dice: 10, count: 1, modifier: 0, bottom: 0, top: 7 });
    expect( mapToRoll( parseClassic( 'd10+3' ))).toMatchObject({ dice: 10, count: 1, modifier: 3 });
    expect( mapToRoll( parseClassic( 'd10-1 (,7)' ))).toEqual({ dice: 10, count: 1, modifier: -1, bottom: 0, top: 7 });
    expect( mapToRoll( parseClassic( '4d10 (9)' ))).toMatchObject({ dice: 10, count: 4, modifier: 0, bottom: 9 });
    expect( mapToRoll( parseClassic( '2d10-2 (,7)' ))).toEqual({ dice: 10, count: 2, modifier: -2, bottom: 0, top: 7 });
  });
});
